...
ui <- navbarPage(
    ...
    wellPanel(
      ...
      p("This",
        ...
        a(HTML(paste(strong("brms"))),
          href = "https://paulbuerkner.com/brms/",
          target = "_blank"),
      ...
      p("The following conventions are used throughout this app:",
        tags$ul(
          ...
          tags$li("Names of R functions are given as",
                  code("<package>::<function>()", .noWS = "after"), ", with",
                  code("<package>"), "denoting the R package which",
                  "contains the", code("<function>", .noWS = "after"),
                  ". Where possible, this scheme is hyperlinked to a",
                  "corresponding documentation website.")
        ))
    ),
    # hr(),
    icon = icon("house")
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        helpText(
          p("Choose an example dataset or upload a file (preferably a",
            "*.csv, *.txt, or *.dat file) containing a custom dataset.",
            "In either case, a preview of the dataset will be shown in the",
            "main panel on the right."),
          p("If a dataset should be uploaded after an example dataset was chosen,",
            "the input field \"Choose example dataset ...\" has to be cleared first."),
  ...
  tabPanel(
    ...
    navlistPanel(
      ...
      tabPanel(
        ...
        titlePanel("Outcome"),
        br(),
        helpText("Choose the outcome (also known as response or dependent",
                 "variable) and the distributional family for this outcome."),
        ...
        selectInput("dist_sel", "Distributional family for the outcome:",
                    ...
                    selectize = TRUE),
        checkboxInput("show_advFams",
                      label = paste("Show advanced distributional families",
                                    "(extends the options in the drop-down",
                                    "list for the distributional family)")),
      ...
      tabPanel(
        ...
        br(),
        helpText(
          p("Choose the predictors variables (also known as explaining or",
            "independent variables) and define how they should be included in",
            "the model: with main effects only, with interaction effects, or",
            "as offsets. The resulting columns in the model matrix are",
            "termed", em("predictors"), "here, in contrast to the predictor",
            "variables which denote columns in the dataset."),
          p("Before an interaction term can be defined, the main-effect terms",
            "of all predictor variables involved in this interaction need to",
            "be included. An overall intercept is always included."),
          p("Numeric variables (with \"numeric\" including \"integer\") are",
            "treated as continuous predictor variables. Non-numeric variables",
            "are treated as nominal predictor variables. The type of a",
            "variable may be seen on page", actionLink("data_link2", "Data"),
            "when choosing the \"Structure\" preview type. If a numeric",
            "variable should be treated as a nominal predictor variable, it",
            "first needs to be converted to a character variable (in the",
            "dataset), e.g., by changing the value", code("1"), "to",
            code("level1", .noWS = "after"), ", the value", code("2"), "to",
            code("level2", .noWS = "after"), ", and so on.",
            "For nominal predictor variables, the first level in alphabetical",
            "order will be the reference level.")
        ),
        wellPanel(
          h3("Main effects"),
          helpText(
          ...
          helpText(
            "Start typing or click into the field below to choose variables",
            "for which group-level main effects should be added.",
            "Note that group-level main effects may not be specified for a",
            "numeric variable. This is not allowed to point out that a",
            "variable must be treated as categorical to have group-level main",
            "effects. If group-level main effects for a numeric variable are",
        ...
        wellPanel(
          ...
          helpText(
            ...
            p("Only variables already being included with a main-effect term",
              "may be included in an interaction term. In the rare case that",
              "an interaction involving a variable without a main-effect term",
              "is really needed, this interaction needs to be included manually",
              "as a variable in the dataset and then a main-effect term for",
              "this manually created interaction variable needs to be added."),
            p("Start typing or click into the field below to choose variables",
              "for which an interaction term should be added. Confirm this",
              "interaction term by pressing the \"Add interaction term\" button.",
              "All interaction terms which have been added are listed in the",
              "box below the \"Add interaction term\" button. This list may be",
        ...
        wellPanel(
          ...
          helpText(
            p("An offset is a predictor with a coefficient fixed to 1.",
              ...
              "In the context of this app, the typical use case would be a",
              "count data outcome where the observation time differs from",
              "individual to individual (see the",
              ...
              "for an example)."),
            p("Start typing or click into the field below to choose variables",
              "for which offsets should be added.")
          ),
    ...
    navlistPanel(
      ...
      tabPanel(
        ...
        helpText(withMathJax(
          p("Before using the Stan results for posterior analyses, it is",
            "important to check the convergence and efficiency of the sampling",
            "algorithm for the given model and data by the help of",
            em("MCMC diagnostics", .noWS = "after"), ".",
            "The MCMC diagnostics used here may be divided into",
            em("HMC-specific diagnostics"), "and",
            em("general MCMC diagnostics", .noWS = "after"), ".",
            "Lists of these two groups of MCMC diagnostics may be found below,",
            "together with some basic guidelines for their interpretation.",
            "These basic guidelines are also checked automatically. Note that",
            "these are", em("basic"), "guidelines",
            HTML("&mdash;", .noWS = "outside"),
            "false positive and false negative alarms are possible and in some",
            "situations, false alarms are more likely than in others.",
            "For details concerning the MCMC diagnostics used here, see",
            a("\"How to Diagnose and Resolve Convergence Problems\"",
              href = "https://mc-stan.org/learn-stan/diagnostics-warnings.html",
              target = "_blank",
              .noWS = "after"),
            ",",
            a("Betancourt (2018)",
              href = "https://doi.org/10.48550/arXiv.1701.02434",
              target = "_blank",
              .noWS = "after"),
            ", and",
            a("Vehtari et al. (2021)",
              ...
              target = "_blank",
              .noWS = "after"),
            ". The document \"How to Diagnose and Resolve Convergence Problems\"",
            "covers most of the MCMC diagnostics used here and gives some",
            "advice on what to do if they indicate problems.",
          ...
          p("The HMC-specific diagnostics are:",
            tags$ul(
              ...
              tags$li("the Bayesian fraction of missing information for the",
                      "energy transitions (E-BFMI) of each chain.")
            ),
            "In general, the first two of these diagnostics are worrying if",
            "they are greater than zero (i.e. at least one iteration ending",
            "with a divergence or at least one iteration hitting the",
            "maximum tree depth) and the third diagnostic (E-BFMI) is worrying",
            "if it is smaller than 0.3 for at least one chain."),
          p("The general MCMC diagnostics (computed for each parameter",
            ...
            tags$ul(
              ...
              tags$li(
                "\\(\\text{ESS}_{\\text{Q}50} \\leq 100 \\cdot n_{\\text{chains}}\\)."
              ),
              tags$li(
          ...
          p("Note: If a", code("constant()"), "prior was used (which should",
            ...
            "the \\(\\widehat{R}\\), \\(\\text{ESS}_{\\text{bulk}}\\),",
            "\\(\\text{ESS}_{\\text{tail}}\\),",
            "\\(\\text{ESS}_{\\text{Q}50}\\), \\(\\text{ESS}_{\\text{Q}2.5}\\),",
            "and \\(\\text{ESS}_{\\text{Q}97.5}\\)",
        ...
        wellPanel(
          ...
          strong("Q50-ESS:"), # strong(withMathJax("\\(\\text{ESS}_{\\text{Q}50}\\):")),
          verbatimTextOutput("essQ50_out", placeholder = TRUE),
      ...
      tabPanel(
        ...
        helpText("Notes:",
                 tags$ul(
                   tags$li("Column", code("Estimate"),
                           "contains the posterior median."),
                   tags$li("Column", code("Est.Error"),
                           "contains the posterior median absolute deviation."),
                   tags$li("Column", code("l-95% CI"),
                           "contains the lower boundary of the",
                           "95 % central posterior interval."),
                   tags$li("Column", code("u-95% CI"),
                           "contains the upper boundary of the",
                           "95 % central posterior interval.")
                 )),
        br(),
      ...
      tabPanel(
        ...
        br(),
        helpText(p("Here, posterior summary quantities for a custom mathematical",
                   "(or logical) expression involving at least one parameter",
                   "may be calculated.",
                   "Click", HTML(paste(actionLink("cust_allow_link", "here"))),
                   "for a list of characters and character groups which are",
                   "allowed in the custom expression.",
                   "For details on how to use these characters or character",
                   "groups, see the examples below or",
                   HTML(paste(actionLink("cust_help_link", "these links")),
                        .noWS = "after"),
                   ". Parameter names need to be enclosed in backticks (",
                   code("`", .noWS = "outside"), ").",
                   "The drop-down list below may be used for inserting",
                   "parameter names (directly with enclosing backticks) into",
                   "the custom expression."),
                 p("Fictitious examples for a custom expression:",
                   tags$ul(
                     tags$li(code("`b_age` + `b_age:genderM`")),
                     tags$li(code("log(`sigma`)")),
                     tags$li(code("`b_treatment` > 0.2"))
                   ),
                   "For the latter example, the posterior mean gives the",
        ...
        br(),
        actionButton("cust_act", "Calculate posterior summary quantities",
                     class = "btn-primary"),
        br(),
        br(),
        ...
        helpText("Note: All columns contain", em("posterior"), "summary quantities.",
                 "In particular, the columns starting with \"Q\" contain the",
                 "corresponding posterior percentiles and column \"MAD\" contains",
                 "the posterior median absolute deviation."),
      ...
      tabPanel(
        ...
        helpText(
          p("The package",
            a(HTML(paste(strong("shinystan"))),
              href = "https://mc-stan.org/shinystan/", target = "_blank"),
            "provides a", strong("shiny"), "app offering an interactive",
            "inspection of Stan's output."),
          p("Notes:",
            tags$ul(
              tags$li(
                "In the", strong("shinystan"), "app, the parameter names given",
                "by", strong("brms"), "are used. These are closely related to",
                "the parameter classes listed on page",
                HTML(paste(actionLink("prior_link3", "Prior"))), "and may be",
                "summarized as follows:",
                tags$ul(
                  tags$li(code("b_Intercept"), "is the intercept (with respect",
                          "to the uncentered predictors)."),
                  tags$li("The parameters starting with", code("b_"), "are the",
                          "population-level effects."),
                  tags$li("If a", code("constant()"), "prior was used (which",
                          "should rarely be the case), then the parameters",
                          "starting with", code("par_"), "are internal",
                          "parameters which do not need to be taken into",
                          "account."),
                  tags$li("The parameters starting with", code("r_"), "are",
                          "the group-level effects."),
                  tags$li("The parameters starting with", code("sd_"), "are",
                          "the standard deviations of the group-level effects."),
                  tags$li("The parameters starting with", code("cor_"), "are",
                          "the correlations between the group-level effects of",
                          "the same group."),
                  tags$li(code("lprior"), "is the accumulated log prior density."),
                  tags$li(code("log-posterior"), "is the accumulated log",
                          "posterior density (up to an additive constant)."),
                  tags$li("All other parameters are parameters specific to the",
                          "chosen distributional family for the outcome (see page",
                          HTML(paste(actionLink("outcome_link2",
                                                HTML("Likelihood &rarr; Outcome"))),
                               .noWS = "after"),
                          ").")
                )
              ...
              tags$li(
                HTML(paste0(
                  ...
                  strong("shinystan"), " are automatically created. These are ",
                  "the observed outcome values (object ", code("y"),
                  ") and the corresponding posterior predictive replications ",
                  "(object ", code("y_rep"), "). They may be selected in the ",
                  "respective \"Object from global environment\" input ",
                  "selector under \"DIAGNOSE\" &rarr; \"PPcheck\" &rarr; ",
        ...
        numericInput("seed_PPD",
                     paste("Seed for draws from the posterior predictive",
                           "distribution (leave empty to use a random seed):"),
                     value = NA, step = 1L),
  ...
  navbarMenu(
    ...
    tabPanel(
        ...
        tags$ul(
          ...
          tags$li(strong("Citation:"),
                  ...
                  em("shinybrms: Graphical User Interface ('shiny' App)",
                     "for 'brms'."),
                  "R package, version 1.8.0.9000. URL:",
          ...
          tags$li(strong("Links:"),
                  a("website", href = "https://fweber144.github.io/shinybrms/",
                    target = "_blank", .noWS = "after"), ",",
                  a("CRAN", href = "https://CRAN.R-project.org/package=shinybrms",
                    target = "_blank", .noWS = "after"), ",",
                  a("GitHub", href = "https://github.com/fweber144/shinybrms",
                    target = "_blank"))
        )
      ),
      wellPanel(
        h3("Issues"),
        "In case of questions, feature requests, or bug reports concerning",
        "this app, please open an issue on",
        a("GitHub", href = "https://github.com/fweber144/shinybrms/issues",
          target = "_blank", .noWS = "after"),
        "."
      ),
      wellPanel(
        ...
        a("GPL-3",
          ...
          target = "_blank",
          .noWS = "after"),
        ". However, the", strong("shinybrms"), "package includes other open",
        "source software components. A list of these components (together with",
        "the full copies of the license agreements used by these components)",
        "may be found",
      ...
      wellPanel(
        ...
        tags$ul(
          ...
          tags$li("Windows is a registered trademark of Microsoft Corporation",
                  "in the United States and other countries."),
          tags$li("Firefox is a trademark of the Mozilla Foundation in the",
    ...
    tabPanel(
      ...
      wellPanel(
        ...
        tags$ul(
          ...
          tags$li(HTML(paste0(
            strong("brms"), ": ",
            a("website", href = "https://paulbuerkner.com/brms/", target = "_blank"), ", ",
    ...
    tabPanel(
      ...
      p("Albert J and Hu J (2019).",
        ...
        ". Boca Raton, FL, USA: CRC Press."),
      p("Betancourt M (2018).",
        "A conceptual introduction to Hamiltonian Monte Carlo (Version 2).",
        em("arXiv", .noWS = "after"), ". DOI:",
        a("10.48550/arXiv.1701.02434",
          href = "https://doi.org/10.48550/arXiv.1701.02434",
          target = "_blank", .noWS = "after"),
        "."),
      ...
      p(withMathJax(
        ...
        HTML("&uuml;", .noWS = "outside"), "rkner P-C (2021).",
        "Rank-normalization, folding, and localization: An improved",
        "\\(\\widehat{R}\\) for assessing convergence of MCMC (with discussion).",
        em("Bayesian Analysis", .noWS = "after"), ",",
        em("16", .noWS = "after"), "(2), 667",
        HTML("&#8211;", .noWS = "outside"), "718. DOI:",
        a("10.1214/20-BA1221", href = "https://doi.org/10.1214/20-BA1221",
          target = "_blank", .noWS = "after"),
        "."))
    )
  ),
...
server <- function(input, output, session) {
    ...
    if (identical(input$ex_da_sel, "Arabidopsis")) {
      ...
      if ("." %in% names(da_tmp)) {
        if (!"X." %in% names(da_tmp)) {
          ...
          showNotification(
            HTML(paste(
              "The column name", code("."), "(dot) is not allowed.",
              "Automatically renaming this column to", code("X."), "failed",
              "since there already exists a column",
              code("X.", .noWS = "after"), "."
            )),
            duration = NA,
            type = "error"
  ...
  observeEvent(input$pred_int_add, {
    if (length(input$pred_int_build) > 1L) {
      ...
      pred_int_rv$choices <- pred_int_rv$choices[!duplicated(lapply(pred_int_rv$choices,
                                                                    sort))]
  ...
  # effect (either population-level or group-level):
  observeEvent({
    input$pred_mainPL_sel
    input$pred_mainGL_sel
  }, {
  ...
  C_pred <- reactive({
    if (is.null(input$pred_mainPL_sel) && is.null(input$pred_mainGL_sel)) {
      ...
      return(data.frame("from_mainGL" = factor(NA_character_,
    ...
    if (length(input$pred_int_sel) > 0L) {
      ...
      #   - Expand interactions on the group-level side (in principle, this is
      #     not necessary as the "*" syntax (<predictor_1>*<predictor_2>) also
      #     works on the group-level side; however, for including correlations
      #     between the group-level effects of a specific group-level term, the
      #     terms on the population-level side need to be grouped by the term on
      #     the group-level side).
      #   - For group-level slopes, add the corresponding population-level slopes
      #     since the group-level slopes are assumed to have mean zero.
      # The first task is performed by applying combn() to
      # m = 1L, ..., length(xGL) with "xGL" containing the group-level terms of
      # a given element of "pred_lst".
      # The second task is performed by additionally applying combn() to m = 0L
      # when performing the first task.
      pred_needsExpand <- sapply(pred_lst, function(x) {
        sum(x %in% input$pred_mainGL_sel) > 0L
      })
      if (any(pred_needsExpand)) {
        # This if () condition is not necessary, but included for better
        # readability.
        ...
        pred_lst_expanded <- do.call("c", lapply(pred_lst_toExpand, function(x) {
          ...
          xGL_lst_expanded <- unlist(lapply(c(0L, seq_along(xGL)), combn,
                                            x = xGL, simplify = FALSE),
                                     recursive = FALSE)
      ...
      # By group-level term: Check each population-level term for being a
      # "subterm" (lower-order term) of a high-order term and if yes, remove it:
      ...
      pred_vec_chr <- factor(pred_vec_chr,
                             levels = unique(pred_vec_chr),
                             exclude = NULL)
    ...
    pred_DF$from_mainGL <- factor(pred_DF$from_mainGL,
                                  levels = unique(pred_DF$from_mainGL),
                                  exclude = NULL)
  ...
  C_prior_default <- reactive({
    ...
    if (inherits(C_prior_default_tmp, "try-error")) {
      ...
      # To avoid running Stan in this case, throw a silent error instead of
      # having `return(brms::empty_prior())`:
      req(FALSE)
    }
    if (length(warn_capt) > 0L) {
      warn_capt <- unique(warn_capt)
      warn_capt[warn_capt == paste(
        "Warning: Rows containing NAs were excluded from the model."
      )] <- paste(
        "Warning: There are missing values in the data. The corresponding rows",
        "have been omitted in the construction of the default priors. They",
        "will also be omitted when running Stan (and also in the Stan data)."
      )
    ...
    return(
      names(C_prior_default()) == "source" |
        (sapply(C_prior_default(), function(x) {
          ...
          !grepl("^prior$|^class$|^coef$|^group$|^lb$|^ub$",
                 names(C_prior_default())))
    )
  })
  ...
  C_stancode <- reactive({
    ...
    if (length(warn_capt) > 0L) {
      ...
      warn_capt[warn_capt == paste(
        ...
        paste("Warning: There are missing values in the data. The",
              "corresponding rows will be omitted when running Stan (and also",
              "in the Stan data).")
      for (warn_capt_i in warn_capt) {
        showNotification(
          warn_capt_i,
  ...
  C_standata <- reactive({
    ...
    if (length(warn_capt) > 0L) {
      ...
      warn_capt[warn_capt == paste(
        ...
        paste("Warning: There are missing values in the data. The",
              "corresponding rows have been omitted in the Stan data. They",
              "will also be omitted when running Stan.")
      for (warn_capt_i in warn_capt) {
        showNotification(
          warn_capt_i,
          duration = NA,
  ...
  observeEvent(input$run_stan, {
    ...
    if (input$advOpts_open_progress) {
      ...
      prog_browser <- getOption("shinybrms.prog_browser", getOption("browser"))
      if (is.function(prog_browser) &&
          any(grepl("rs_browseURL|rs_shinyviewer",
                    as.character(body(prog_browser))))) {
        ...
        # initialization of the R session), so use the default browser stored in
      ...
      # to be used or actually used (see the source code from `library(rstan);
      # getMethod("sampling", signature = "stanmodel")` for the condition when
      # parallel::mclapply() with forking is used) or if using the **cmdstanr**
      # backend:
      if (identical(.Platform$OS.type, "unix") &&
        ...
        # avoid forking):
        tmp_stdout_txt <- tempfile(pattern = "shinybrms_stdout_",
                                   fileext = ".txt")
        sink(tmp_stdout_txt)
        sink_active <- TRUE
    ...
    # Run Stan (more precisely: brms::brm() (or brms:::update.brmsfit(), if
    # possible)):
    if (use_upd) {
    ...
    if (length(warn_capt) > 0L) {
      ...
      warn_capt[warn_capt == paste(
        ...
        paste("Warning: There were missing values in the dataset which was",
  ...
  C_stanres <- reactive({
    ...
    C_rhat <- apply(C_draws_arr, MARGIN = 3,
                    FUN = if (requireNamespace("posterior", quietly = TRUE)) {
                      posterior::rhat
                    } else {
                      rstan::Rhat
    ...
    C_essQ50 <- apply(
      C_draws_arr, MARGIN = 3,
      FUN = if (requireNamespace("posterior", quietly = TRUE)) {
        # A shortcut would be posterior::ess_median():
        function(x_sims) posterior::ess_quantile(x_sims, probs = c(0.5))
      } else {
        function(x_sims) rstan:::ess_quantile(x_sims, prob = 0.5)
      }
    )
    if (any(is.na(C_essQ50))) {
      C_essQ50_OK <- FALSE
    } else {
      C_essQ50_OK <- all(C_essQ50 > 100 * n_chains_out)
    }
    ...
    C_all_OK <- all(c(C_div_OK, C_tree_OK, C_EBFMI_OK,
                      ...
                      C_essQ50_OK, C_essQ2.5_OK, C_essQ97.5_OK))
    ...
    if (!C_bfit_raw()$is_upload && n_chains_out < C_bfit_raw()$n_chains_spec) {
      showNotification(
        paste("Warning: Stan results obtained, but at least one chain exited",
              "with an error. The Stan results should not be used."),
        duration = NA,
        type = "warning"
      )
    } else {
      ...
      if (C_all_OK) {
        ...
        showNotification(
          paste("Warning: Stan results obtained, but at least one MCMC",
                "diagnostic is worrying (see the tab \"MCMC diagnostics\" for",
                "details). In general, this indicates that the Stan results",
                "should not be used."),
          duration = NA,
          type = "warning"
        )
  ...
  output$essQ50_out <- renderText({
    input$run_stan # Just for graying out.
    invisible(req(C_stanres()))
    if (C_stanres()$diagn$ESS_Q50_OK) {
      return("All Q50-ESS values are OK.")
    } else {
      return(paste("Warning: At least one Q50-ESS value is worrying. In general,",
                   "this indicates that the posterior medians should not be used."))
    }
  }, sep = "\n")
  ...
  output$essQ2.5_out <- renderText({
    ...
    if (C_stanres()$diagn$ESS_Q2.5_OK) {
      ...
      return(paste("Warning: At least one Q2.5-ESS value is worrying. In general,",
                   "this indicates that the central 95 % posterior intervals",
                   "should not be used."))
    }
  }, sep = "\n")
  ...
  output$general_MCMC_out <- renderPrint({
    ...
    data.frame("R-hat" = C_stanres()$diagn$Rhat,
               ...
               "ESS_Q50" = C_stanres()$diagn$ESS_Q50,
  ...
  observeEvent(input$cust_act, {
    ...
    C_cust(rbind(C_cust(), with(as.data.frame(C_draws_mat()), {
      ...
      cust_smmry <- cbind(data.frame("Name" = input$cust_name),
                          as.data.frame(t(cust_q)), data.frame(
                            "MAD" = mad(cust_res),
                            "mean" = mean(cust_res),
                            "SD" = sd(cust_res)
                          ))
    ...
    if (requireNamespace("shinystan", quietly = TRUE)) {
      if (requireNamespace("callr", quietly = TRUE)) {
        ...
        if (is.function(shinystan_browser) &&
            any(grepl("rs_browseURL|rs_shinyviewer",
                      as.character(body(shinystan_browser))))) {
          # In this case, "shinystan_browser" cannot be used (at least not
          # without requiring the user to perform some major modifications to
          # the initialization of the R session), so use the default browser
          # stored in the environment variable "R_BROWSER":
        ...
        # another "shiny" app (here "shinystan") from within the current "shiny"
        # app (here "shinybrms")):
        callr::r(
