...
\
After selecting Tab "Predictors", we choose variables `week` and `trt` to have nonpooled main effects and variable `ID` to have partially pooled main effects:
...
\
We can now switch back to the browser tab where **shinybrms** is running. Most importantly, we get the result of an "overall check" for the Markov chain Monte Carlo (MCMC) diagnostics which are given in detail in Tab "MCMC diagnostics". Here, all MCMC diagnostics are OK:
