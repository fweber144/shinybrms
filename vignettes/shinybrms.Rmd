...
\
Underneath the HMC diagnostics, we get a short summary of our posterior inference including some further Markov chain Monte Carlo (MCMC) diagnostics (`Rhat`, `Bulk_ESS`, `Tail_ESS`). Here, these MCMC diagnostics are all OK (see [here](https://arxiv.org/abs/1903.08008) why):
...
\
For those who would now want to start interpreting the posterior results given under "Summary" (e.g. the posterior median and the posterior 2.5 \% and 97.5 \% quantiles), it should be noted that the output shown under "Summary" is only intended for a quick inspection. A by far more comprehensive analysis of the output (including the diagnostics and also posterior predictive checks) is possible using the [**shinystan**](https://CRAN.R-project.org/package=shinystan) package which provides a Shiny app for inspecting the output from Stan model fits (like ours). Here, we launch the **shinystan** app by clicking the corresponding button at the bottom of the page:
...
\
Here, the **shinybrms** workflow ends and passes over to the **shinystan** workflow. Note that within the **shinybrms** app, you can download multiple output objects (see Fig. 18) to analyze them outside of **shinybrms**.
